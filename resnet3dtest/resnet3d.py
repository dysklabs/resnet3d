...
def _bn_relu(input):
    ...
    norm = GroupNormalization(groups=16, axis=CHANNEL_AXIS)(input) 
    return LeakyReLU(alpha=0.2)(norm) #Activation("relu")(norm)
...
def _shortcut3d(input, residual):
    ...
    stride_dim2 = input._keras_shape[DIM2_AXIS]         // residual._keras_shape[DIM2_AXIS]
    stride_dim3 = input._keras_shape[DIM3_AXIS]         // residual._keras_shape[DIM3_AXIS]
...
def basic_block(filters, strides=(1, 1, 1), kernel_regularizer=l2(1e-8),
...
def bottleneck(filters, strides=(1, 1, 1), kernel_regularizer=l2(1e-8),
               is_first_block_of_first_layer=False):
    """Basic 3 X 3 X 3 convolution blocks. Extended from raghakot's 2D impl."""
    def f(input):
        if is_first_block_of_first_layer:
...
class Resnet3DBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn, repetitions, reg_factor):
        ...
        # first conv
        conv1 = _conv_bn_relu3D(filters=64, kernel_size=(3, 3, 3), padding='same',
                                #strides=(2,2,8),
                                strides=(1, 1, 1),
                                kernel_regularizer=l2(reg_factor)
                                )(input)
        pool1 = MaxPooling3D(strides=(2, 2, 2),
                             padding="same")(conv1)
        block = pool1
#         block = conv1
